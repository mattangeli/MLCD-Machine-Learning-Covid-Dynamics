...
def SAIVR_solution(t, s_0, a_0, i_0, v_0, r_0, alpha_1, beta_1, gamma, delta, parameters_fixed):
    # Scipy Solver
...
def test_fitmodel(model, data_type, time_series_dict, optimized_params, average = 'D', lineW = 3):
   ...
   plt.title(' $a_0$= {:.2f}, $i_0$={:.2f}, $r_0$={:.2f}, alpha_1={:.2f}, beta_1={:.2f}, $\gamma$={:.2f}, $\delta$={:.2e}'.format(round(a0,2),
              round(i0,2), round(r0,2), round(alpha1,2), round(beta1,2), round(gamma0,2), round(delta0,2)))     
   plt.legend()
...
def test_snippet(model, epoch, loss, t_0, t_final, params_fixed, a0, i0, v0, r0, alpha_1=0.15, beta_1=0.1,
   ...
   plt.legend()  
   plt.title('a0={:.2f}, i0={:.2f}, r0={:.2f}, alpha1={:.2f}, beta1={:.2f}, gamma={:.2f}, delta={:.2e}'.format(a0.item(),i0.item(),r0.item(), alpha_1.item(), beta_1.item(), gamma.item(), delta.item())) 
...
def test_model(model, initial_conditions_set, parameters_bundle, params_fixed, t_0, t_final, lineW = 3, n_test = 1000):  
   ...
   i00 = uniform(initial_conditions_set[1][0], initial_conditions_set[1][1], size=1)
   v00 = uniform(initial_conditions_set[2][0], initial_conditions_set[2][1], size=1)
   r00 = uniform(initial_conditions_set[3][0], initial_conditions_set[3][1], size=1)
