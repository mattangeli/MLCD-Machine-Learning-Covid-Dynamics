...
def fit_data_real(model, time_series_dict, initial_conditions_set, parameters_bundle, param_fixed, lr, epochs,
    ...
    optimizer = torch.optim.Adam([i_0, a_0, v_0, r_0, alpha_1, beta_1, gamma], lr=lr)
    ...
    for epoch in tqdm(range(epochs), desc='Finding the best parameters/initial conditions'):  
        ...
        for t in time_sequence:
            ...
            loss_i = data_fitting_loss(data_type, t_tensor, true_pop, s_hat, a_hat, 
                                                                   i_hat, v_hat, r_hat)
            # regularization to avoid negative populations in extreme cases
            regularization = torch.exp(-0.1 * i_0)  + torch.exp(-0.1 * a_0)                                                 
    ...
    return best_model, loss_history, s_0.item(), a_0.item(), i_0.item(), v_0.item(), r_0.item(), alpha_1.item(), beta_1.item(), gamma.item()
...
def train_saivrNet(model, optimizer, scheduler, t_0, t_final, initial_conditions_set, parameters_bundle, parameters_fixed, epochs,
               train_size, num_batches, hack_trivial, decay, model_name, ROOT_DIR, loss_threshold = float('-inf')):
